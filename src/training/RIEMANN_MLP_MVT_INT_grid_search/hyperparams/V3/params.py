...
params_dict_lists = { #V1
    'EPOCHS_TMIN': [0], # 3 -> start time of the epoch from the movement onset (in seconds)
    'EPOCHS_LENGTH': [2], # 3 -> length of the epoch (in seconds) [TO BE USED IF NO EPOCHS_TMAX IS USED]
    # 'EPOCHS_TMAX': [0, 0.5, 1], # 3 -> end time of the epoch from the movement onset (in seconds) [TO BE USED IF NO EPOCHS_LENGTH IS USED]
    'EPOCHS_EMPTY_FROM_MVT_TMIN': [-6], # 3 -> start time of the empty epoch from the movement onset
    'EPOCHS_INTENTION_FROM_MVT_TMIN': [-2], # 2 -> OPTIONAL: start time of the intention epoch from the movement onset
