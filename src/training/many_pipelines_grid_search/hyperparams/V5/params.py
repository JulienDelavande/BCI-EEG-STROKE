...
params_dict_lists = { #V1
    'EPOCHS_TMIN': [-0.75, -0.5, -0.25], # 1 -> start time of the epoch from the movement onset (in seconds)
    'EPOCHS_LENGTH': [2, 1.6, 1.4, 1, 0.5], # 5 -> length of the epoch (in seconds) [TO BE USED IF NO EPOCHS_TMAX IS USED]
    # 'EPOCHS_TMAX': [0, 0.5, 1], # 3 -> end time of the epoch from the movement onset (in seconds) [TO BE USED IF NO EPOCHS_LENGTH IS USED]
    'EPOCHS_EMPTY_FROM_MVT_TMIN': [-8], # 1 -> start time of the empty epoch from the movement onset
    'EPOCHS_INTENTION_FROM_MVT_TMIN': [-4, -1.2, -1, -0.5], # 4 -> OPTIONAL: start time of the intention epoch from the movement onset
    'FMIN': [1, 5, 9, 12, 15, 20, 30], #  7 -> min frequency
    'FMAX': [20, 25, 30, 35, 40, 50, 60], # 7 -> max frequency
